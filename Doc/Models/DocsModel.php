...
class DocsModel
{
    ...
    protected function isPropertyMethod($method)
    {
        ...
        return $method->getNumberOfRequiredParameters() === 0 && !$method->isStatic()
        && strncasecmp($methodName, 'get', 3) === 0 && isset($methodName[3]);
    }
        ...
        foreach ($fileContent as $no => $line) {
            ...
            if (preg_match('/^\s*\w+[\s\w]*\sfunction\s\w+\((.+)\)/', $line, $matches, PREG_OFFSET_CAPTURE)) {
                ...
                foreach ($params as $br => $param) {
                    ...
                    /*
                     * Strip default value if exists ex. data=[] (with spaces)
                     */
